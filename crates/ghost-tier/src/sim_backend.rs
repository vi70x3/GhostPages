...
//!
//! This module provides `SimBackend`, a RAM-based simulation backend that
//! mimics memory tier behavior with configurable latency, bandwidth limits,
//! fragmentation, and failure injection. It is used both as a standalone
//! simulation backend and as the simulation layer within `DiskBackend`.
//!
...
use crate::sim_metrics::SimMetrics;
...
/// given the same seed.
///
/// This backend is used in two ways:
/// 1. As a standalone simulation backend (via `ghost-sim` crate)
/// 2. As the simulation layer within `DiskBackend` for latency/pressure/health
#[derive(Debug)]
...
impl SimBackend {
    ...
    pub async fn simulate_latency(&self, bytes: usize) {
...
mod tests {
    ...
    async fn test_sim_backend_failure_injection() {
        let failure_config = crate::sim_config::FailureConfig {
            ...
            failure_pattern: crate::sim_config::FailurePattern::Random,
        };
